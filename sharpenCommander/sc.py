...
class MyProgram(Program):
    ...
    def regRemove(self, pp, cb=None):
        ...
        def onOk():
            ...
            # self.fileRefresh()
            popupMsg(
                "Unregister", "The path is unregisterted successfully\n%s" % pp, 60
            )
            if cb is not None:
                cb(True)
        ...
        popupAsk(
            "Unregister",
            "Do you want to unregister the folder?\n%s" % pp,
            onOk,
            onCancel,
        )
    ...
    def gitPush(self):
            ...
            if self.isPushRebase:
                ...
                if revCommon.startswith(rev2):
                    ...
                    if len(diffList) == 0:
                        while True:
                            hr = input(
                                "\n\n*** You can rebase local to remoteBranch. want? Y/n: "
                            ).lower()
                            if hr == "":
                                hr = "y"
                            ...
                            if hr == "n":
                                ...
                                if st != 0:
                                    git.rebaseAbort()
                                    raise Exception(
                                        "rebase failed. you should manually merge it.[err:%d]"
                                        % st
                                    )
                                break
                    else:
                        while True:
                            hr = input(
                                "\n\n*** It could be impossible to rebase onto remoteBranch. rebase/skip: "
                            ).lower()
                            if hr == "rebase":
        ...
        # push it
        print(
            "\n** Current branch is pushing to the remote branch[%s/%s]..."
            % (remote, target)
        )
        ...
        if status != 0:
            while True:
                hr = input(
                    "\n\nPush failed. Do you want to push with force option?[y/N]: "
                ).lower()
                if hr == "y":
...
class mDlgMainDc(cDialog):
    def __init__(self):
        ...
        self.widgetFileList = mListBox(
            urwid.SimpleFocusListWalker(btnListMakeTerminal([], None))
        )
        # attrFileList = urwid.AttrMap(self.widgetFileList, "std")
        self.widgetCmdList = mListBox(
            urwid.SimpleFocusListWalker(btnListMakeTerminal([], None))
        )  # 이거 파일목록 아래에뭔가 보여주는건데..
        self.widgetContent = urwid.Pile(
            [self.widgetFileList, ("pack", urwid.Divider("-")), (8, self.widgetCmdList)]
        )
        self.widgetContent.isShow = True

        # extra
        self.widgetWorkLabel = urwid.Text("< Workspace >")
        self.widgetWorkList = mListBox(
            urwid.SimpleFocusListWalker(btnListMakeTerminal([], None))
        )
        self.widgetTempLabel = urwid.Text("< Attr >")
        self.widgetTempList = mListBox(
            urwid.SimpleFocusListWalker(btnListMakeTerminal([], None))
        )
        self.widgetExtraList = urwid.Pile(
            [
                ("pack", self.widgetWorkLabel),
                self.widgetWorkList,
                ("pack", self.widgetTempLabel),
                self.widgetTempList,
            ]
        )
        ...
        self.widgetFrame = urwid.Columns(
            [("weight", 1, self.widgetContent), (20, self.widgetExtraList)]
        )
        self.edInput = editGen(
            "$ ", "", lambda edit, text: self.onInputChanged(edit, text)
        )
        self.mainWidget = urwid.Frame(
            self.widgetFrame, header=self.headerText, footer=self.edInput
        )
    ...
    def cmdShow(self, lstItem):
        ...
        if isShow != self.widgetContent.isShow:  # 이거 자체는 왼쪽 컨텐츠다.
            ...
            if isShow:
                ...
                self.widgetContent.contents[1] = (
                    urwid.Divider("-"),
                    (urwid.widget.PACK, None),
                )
                self.widgetContent.contents[2] = (
                    self.widgetContent.contents[2][0],
                    (urwid.widget.GIVEN, 8),
                )
            else:
                # self.widgetContent.contents[1] = (self.widgetContent.contents[1][0], (urwid.widget.GIVEN, 0))  # 이게 잘안된다. 아마 divider는 pack만 지원하는듯
                self.widgetContent.contents[1] = (
                    urwid.Pile([]),
                    (urwid.widget.GIVEN, 0),
                )
                self.widgetContent.contents[2] = (
                    self.widgetContent.contents[2][0],
                    (urwid.widget.GIVEN, 0),
        ...
        refreshBtnListMarkupTuple(
            lstItem, self.widgetCmdList, lambda btn: self.onFileSelected(btn)
        )
    ...
    def gotoRefresh(self, newText):
        ...
        refreshBtnListMarkupTuple(
            lst, self.widgetFileList, lambda btn: self.onFileSelected(btn)
        )
    ...
    def fileRefresh(self, newText=None):
        ...
        if filterStr == "":
            if g.targetFile is not None:
                ...
                if self.lastPath == curPath:
                    ...
                    self.lastPath is not None
                    and os.path.dirname(self.lastPath) == curPath
                ):
        ...
        self.widgetFileList.body += btnListMakeMarkup(
            itemList, lambda btn: self.onFileSelected(btn)
        )
    ...
    def changePath(self, pp, newCmd=""):
        ...
        try:
            ss = subprocess.check_output(
                ["git", "branch", "--color=never"], stderr=subprocess.DEVNULL
            ).decode()
        ...
        if self.cmd == "find":
            ...
            if self.mainWidget.get_focus() == "body":
                ...
                if self.cmd == "goto":
                    ...
                    print(ss)
                    input(
                        "Enter to return...(exit code:%d)" % code
                    )  # TODO: support esc key?
                    g.loop.start()
                    self.fileRefresh()
                    ...
                    if ss == "list":
                        ...
                        if item is not None:
                            ...
                            popupMsg(
                                "Regiter the folder",
                                "The path is already registerted\n%s" % pp,
                                60,
                            )
                            return
                        ...
                        popupMsg(
                            "Set repo status",
                            "The path is set as %s\n%s"
                            % ("Repo" if item["repo"] else "Not Repo", pp),
                            60,
                        )
                        return
                    else:
    ...
    def workRefresh(self):
        ...
        self.widgetWorkList.body += btnListMakeMarkup(
            itemList, lambda btn: self.onFileSelected(btn)
        )
    ...
    def unhandled(self, key):
        ...
        # 	popupMsg("Dc help", "Felix Felix Felix Felix\nFelix Felix")
            ...
            key == "j"
        ):  # we can't use ctrl+j since it's terminal key for enter replacement
            self.widgetFileList.focusNext()
        elif key == "k":
            self.widgetFileList.focusPrevious()
        elif key == "J":
    ...
    def doGrep(self, target):
        ...
        cmds = [which(g.grepApp), "--group", "--color", target]
        subMake(dlg, cmds)
    ...
    def checkSameWith(self, name, branchName, remoteBranch):
        ...
        if isSame:
            ...
            self.log2(
                Color.red,
                name,
                "Your local branch(%s) is forward than %s[%d commits]"
                % (branchName, remoteBranch, gap),
            )
        ...
        if not name.startswith("/"):
            ...
            if "type" in repo and repo["type"] == "bin":
                self.log2(
                    Color.blue,
                    name,
                    "merge with %s - %s - bin type" % (remoteBranch, path),
                )
...
def main():
  ...
  """
    ...
    # 	return
        ...
        print(
            "%s[%s] - %s[%s] - %d - %d"
            % (branch, rev, upstream, remoteRev, ahead, behind)
        )
        return 1
