...
class mButton(urwid.Button):
    ...
    def __init__(self, label, on_press=None, user_data=None):
        ...
        self._label = urwid.SelectableIcon(label, 0)
...
class mListBox(urwid.ListBox):
    def __init__(self, body):
        ...
        if getattr(self.body, "set_focus_changed_callback", None):
            self.body.set_focus_changed_callback(
                lambda newFocus: self.onFocusChanged(newFocus)
            )
...
def btnListMakeMarkup(lstMarkup, onClick, isFirstFocus=True, doApply=None):
    ...
    for markup, text, attr in lstMarkup:
        ...
        btn = btnGen(
            (markup, text), ("std_f", text), attr, onClick, isFirstFocus, doApply
        )
        isFirstFocus = False
        outList.append(btn)
...
def popupMsg(title, ss, width=50):
    ...
    g.loop.widget = urwid.Overlay(
        urwid.Filler(popup), g.loop.widget, "center", width, "middle", 10
    )
...
def popupAsk(title, ss, onOk, onCancel=None):
    ...
    popup = urwid.LineBox(
        urwid.Pile([("pack", txtMsg), ("pack", urwid.Columns([btnYes, btnNo]))]), title
    )
    g.loop.widget = urwid.Overlay(
        urwid.Filler(popup), g.loop.widget, "center", 40, "middle", 5
    )
...
def popupAsk3(title, ss, btnName1, btnName2, btnName3, onBtn1, onBtn2, onBtn3=None):
    ...
    popup = urwid.LineBox(
        urwid.Pile([("pack", txtMsg), ("pack", urwid.Columns([btnB1, btnB2, btnB3]))]),
        title,
    )
    g.loop.widget = urwid.Overlay(
        urwid.Filler(popup), g.loop.widget, "center", 40, "middle", 5
...
def popupInput(title, ss, onOk, onCancel=None, width=40):
    ...
    popup = urwid.LineBox(
        urwid.Pile([("pack", edInput), ("pack", urwid.Columns([btnOk, btnCancel]))]),
        title,
    )
    g.loop.widget = urwid.Overlay(
        urwid.Filler(popup), g.loop.widget, "center", width, "middle", 5
    )
