...
class DlgGitCommit(cDialog):
    ...
    def __init__(self, onExit):
        ...
        self.edInput = editGen(
            ...
            "",
            lambda edit, text: self.onMsgChanged(edit, text),
        )
        self.widgetFileList = mListBox(
            urwid.SimpleFocusListWalker(
                btnListMakeTerminal([("< No files >", None)], None)
            )
        )
        self.widgetCommitList = mListBox(
            urwid.SimpleListWalker(textListMakeTerminal(["< There is no commit >"]))
        )
        self.widgetContent = mListBox(
            urwid.SimpleListWalker(textListMakeTerminal(["< Nothing to display >"]))
        )

        self.headerText = urwid.Text(
            ">> Commit - f9/f10(Prev/Next file) f4(cancel operation)"
        )
        self.widgetFrame = urwid.Pile(
            [
                ("pack", self.edInput),
                (8, urwid.AttrMap(self.widgetFileList, "std")),
                ("pack", urwid.Divider("-")),
                (4, self.widgetCommitList),
                ("pack", urwid.Divider("-")),
                self.widgetContent,
            ]
        )
    ...
    def _applyFileColorTheme(self, widget, isFocus=0):
        ...
        widget.original_widget.set_label(
            (theme[isFocus], widget.original_widget.origTxt)
        )
    ...
    def onFileSelected(self, btn):
        ...
        try:
            ss = system(
                'git diff --color %s "%s"' % ("" if btnType == "c" else "--staged", pp)
            )
    ...
    def refreshCommitList(self):
        ...
        refreshBtnListTerminal(
            lst, self.widgetCommitList, lambda btn: self.onFileSelected(btn)
        )
        return True
    ...
    def refreshFileList(self):
        ...
        itemList = [
            (self.themes[0][0], x, "s") for x in fileList.split("\n") if x.strip() != ""
        ]
        ...
        itemList = [
            (self.themes[1][0], x, "c") for x in fileList.split("\n") if x.strip() != ""
        ]
        self.widgetFileList.body += btnListMakeMarkup(
            itemList, lambda btn: self.onFileSelected(btn), False
        )
        ...
        if len(self.widgetFileList.body) == 0:
            self.widgetFileList.body += btnListMakeTerminal(
    ...
    def unhandled(self, key):
        if key == "q" or key == "Q" or key == "f4":
            ...
            popupAsk3(
                "Git add",
                "Do you want to add a file[%s]?" % fname,
                "Add",
                "Prompt",
                "Cancel",
                onAdd,
            ...
            popupAsk(
                "Git reset(f)",
                "Do you want to drop file[%s]s modification?" % fname,
                onDrop,
            )
...
class DlgGitStatus(cDialog):
    ...
    def unhandled(self, key):
        if key == "f4" or key == "q":
            ...
            fname = gitFileBtnName(btn)
            popupAsk(
                "Git add", "Do you want to add a file via prompt[%s]?" % fname, onPrompt
            )
